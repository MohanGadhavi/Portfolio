...
const Experience = () => {
  return (
    <section
      id="experience"
      className="py-12 md:py-20 bg-gray-50 dark:bg-black transition-colors duration-300"
    >
              ...
              <div className="md:flex items-start gap-12">
                <div className="hidden md:block self-stretch relative">
                  <div className="absolute top-12 h-full md:flex flex-col items-center">
                    <div className="w-4 h-4 rounded-full bg-accent z-10 ring-4 ring-gray-50 dark:ring-black shrink-0" />
                    <div
                      className={`w-0.5 bg-gray-200 dark:bg-white/10 ${index === experiences?.length - 1 ? " min-h-[calc(90%-2rem)] " : "  min-h-[110%] "} `}
                    />
                  </div>
                </div>

                <div className="flex-1 bg-white dark:bg-white/5 hover:bg-gray-800 p-8 rounded-2xl border border-slate-300 hover:border-slate-400 dark:border-white/10 transition-all ease-in-out duration-1000 shadow-sm hover:shadow-lg group">
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-gray-900 group-hover:text-gray-200 dark:text-white flex items-center gap-2">
                        <Briefcase size={20} className="text-accent" />
                        {exp.role}
                      </h3>
                      <p className="text-accent font-medium">{exp.company}</p>
                    </div>
                    <span className="text-gray-600 dark:text-gray-400 text-sm mt-2 md:mt-0 bg-gray-200 dark:bg-white/10 px-3 py-1 rounded-full">
                      {exp.period}
                    </span>
                  </div>
                  <ul className="space-y-3">
                    {exp.description.map((item, i) => (
                      <li
                        key={i}
                        className="text-gray-600 group-hover:text-gray-200 dark:text-gray-300 text-sm leading-relaxed flex items-start gap-2"
                      >
